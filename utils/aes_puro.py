# utils/aes_puro.py
import random

# ========== UTILIDADES DE PADDING ==========
def _pad(data):
    pad_len = 16 - (len(data) % 16)
    return data + bytes([pad_len] * pad_len)

def _unpad(data):
    pad_len = data[-1]
    if 1 <= pad_len <= 16 and data[-pad_len:] == bytes([pad_len]) * pad_len:
        return data[:-pad_len]
    raise ValueError("Padding inválido")

# ========== IMPLEMENTACIÓN AES (de Bo Zhu) ==========
Sbox = (
    ,
)

InvSbox = (
    ,
)

xtime = lambda x: (((x << 1) ^ 0x1B) & 0xFF) if (x & 0x80) else (x << 1)

Rcon = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)

def text2matrix(data):
    if isinstance(data, int):
        data = data.to_bytes(16, 'big')
    if isinstance(data, str):
        data = data.encode('utf-8')
    if len(data) > 16:
        raise ValueError("Solo un bloque en text2matrix")
    padded = data.ljust(16, b'\0')
    matrix = []
    for i in range(16):
        byte = padded[i]
        if i % 4 == 0:
            matrix.append([byte])
        else:
            matrix[-1].append(byte)
    return matrix

def matrix2text(matrix):
    data = bytearray()
    for row in matrix:
        data.extend(row)
    return bytes(data)

class AES:
    def __init__(self, master_key):
        if isinstance(master_key, bytes):
            master_key = int.from_bytes(master_key, 'big')
        self.round_keys = None
        self.change_key(master_key)

    def change_key(self, master_key):
        self.round_keys = text2matrix(master_key)
        for i in range(4, 44):
            self.round_keys.append([])
            if i % 4 == 0:
                byte = self.round_keys[i - 4][0] \
                     ^ Sbox[self.round_keys[i - 1][1]] \
                     ^ Rcon[i // 4]
                self.round_keys[i].append(byte)
                for j in range(1, 4):
                    byte = self.round_keys[i - 4][j] \
                         ^ Sbox[self.round_keys[i - 1][(j + 1) % 4]]
                    self.round_keys[i].append(byte)
            else:
                for j in range(4):
                    byte = self.round_keys[i - 4][j] \
                         ^ self.round_keys[i - 1][j]
                    self.round_keys[i].append(byte)

    def encrypt(self, plaintext):
        if isinstance(plaintext, bytes):
            plaintext = int.from_bytes(plaintext, 'big')
        state = text2matrix(plaintext)
        self.__add_round_key(state, self.round_keys[:4])
        for i in range(1, 10):
            self.__sub_bytes(state)
            self.__shift_rows(state)
            self.__mix_columns(state)
            self.__add_round_key(state, self.round_keys[4*i:4*(i+1)])
        self.__sub_bytes(state)
        self.__shift_rows(state)
        self.__add_round_key(state, self.round_keys[40:])
        return matrix2text(state)

    def decrypt(self, ciphertext):
        if isinstance(ciphertext, bytes):
            ciphertext = int.from_bytes(ciphertext, 'big')
        state = text2matrix(ciphertext)
        self.__add_round_key(state, self.round_keys[40:])
        self.__inv_shift_rows(state)
        self.__inv_sub_bytes(state)
        for i in range(9, 0, -1):
            self.__add_round_key(state, self.round_keys[4*i:4*(i+1)])
            self.__inv_mix_columns(state)
            self.__inv_shift_rows(state)
            self.__inv_sub_bytes(state)
        self.__add_round_key(state, self.round_keys[:4])
        return matrix2text(state)

    @staticmethod
    def __add_round_key(s, k):
        for i in range(4):
            for j in range(4):
                s[i][j] ^= k[i][j]

    @staticmethod
    def __sub_bytes(s):
        for i in range(4):
            for j in range(4):
                s[i][j] = Sbox[s[i][j]]

    @staticmethod
    def __inv_sub_bytes(s):
        for i in range(4):
            for j in range(4):
                s[i][j] = InvSbox[s[i][j]]

    @staticmethod
    def __shift_rows(s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]

    @staticmethod
    def __inv_shift_rows(s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]

    @staticmethod
    def __mix_single_column(m_data):
        t = m_data[0] ^ m_data[1] ^ m_data[2] ^ m_data[3]
        u = m_data[0]
        m_data[0] ^= t ^ xtime(m_data[0] ^ m_data[1])
        m_data[1] ^= t ^ xtime(m_data[1] ^ m_data[2])
        m_data[2] ^= t ^ xtime(m_data[2] ^ m_data[3])
        m_data[3] ^= t ^ xtime(m_data[3] ^ u)

    def __mix_columns(self, s):
        for i in range(4):
            self.__mix_single_column(s[i])

    def __inv_mix_columns(self, s):
        for i in range(4):
            u = xtime(xtime(s[i][0] ^ s[i][2]))
            v = xtime(xtime(s[i][1] ^ s[i][3]))
            s[i][0] ^= u
            s[i][1] ^= v
            s[i][2] ^= u
            s[i][3] ^= v
        self.__mix_columns(s)

# ========== CBC MODE ==========
def _aes_cbc_encrypt(plaintext, key):
    plaintext_padded = _pad(plaintext)
    iv = bytes(random.getrandbits(8) for _ in range(16))
    ciphertext = iv
    prev = iv
    for i in range(0, len(plaintext_padded), 16):
        block = plaintext_padded[i:i+16]
        if len(block) < 16:
            block += b'\0' * (16 - len(block))
        block_xor = bytes(a ^ b for a, b in zip(block, prev))
        aes = AES(key)
        encrypted = aes.encrypt(block_xor)
        ciphertext += encrypted
        prev = encrypted
    return ciphertext

def _aes_cbc_decrypt(ciphertext, key):
    if len(ciphertext) < 16 or (len(ciphertext) - 16) % 16 != 0:
        raise ValueError("Cifrado inválido")
    iv = ciphertext[:16]
    encrypted_blocks = ciphertext[16:]
    plaintext = b""
    prev = iv
    for i in range(0, len(encrypted_blocks), 16):
        block = encrypted_blocks[i:i+16]
        aes = AES(key)
        decrypted = aes.decrypt(block)
        plaintext_block = bytes(a ^ b for a, b in zip(decrypted, prev))
        plaintext += plaintext_block
        prev = block
    return _unpad(plaintext)